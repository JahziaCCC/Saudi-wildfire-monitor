...
# 1) PACKAGES & SETUP
required_packages <- c("sf", "leaflet", "dplyr", "readr", "httr2", "jsonlite")
...
# Replace with your NASA FIRMS MAP_KEY if available, or use default open access endpoint
...
for (src in sources) {
  url <- sprintf("https://firms.modaps.eosdis.nasa.gov/api/country/csv/%s/SAU/1", src)
  tryCatch({
...
if (nrow(raw_data) > 0) {
  ...
  # Smart Classification Logic (Wildfire vs Industrial Flare)
  # Distinguishes static industrial gas flares based on confidence and recurrence proxies
  fires_sf <- fires_sf %>%
    mutate(
      ...
      # Risk Calculations (KRI & FWI Proxies)
      fwi_score = round((frp * 0.4) + (bright_ti4 * 0.1) - 25, 1),
      kri_score = round(pmin(pmax((fwi_score * 1.2) + (confidence_num * 0.3), 0), 100), 1)
    )
    
} else {
  stop("No active detections found for Saudi Arabia today.")
}

# 4) EXPORT OUTPUTS (CSV & GeoJSON)
...
send_telegram_alert <- function(token, chat_id, message_text, file_path = NULL) {
  ...
  if (!is.null(file_path) && file.exists(file_path)) {
    ...
    tryCatch(req_perform(req_doc), error = function(e) cat("⚠️ خطأ في إرسال الملف\n"))
  }
}
...
report_text <- paste0(
  ...
  "📁 *مرفق ملف التقرير التفصيلي بصيغة CSV.*"
)
